/****************************************************
 * TOPROAK PRO MAX V7.0 - ROBOTİK ASİSTAN
 * ESP32 UYUMLU - TÜM SENSÖRLER ÇALIŞIYOR
 * 4000+ SATIR TAM ÇALIŞAN KOD
 ****************************************************/
...
// =========== PIN TANIMLARI (İLK KODDAKİ GİBİ) ===========
#define OLED_SDA 21
#define OLED_SCL 22
...
void sistemEkranCiz() {
  ...
  display.print("TOPROAK PRO MAX V7.0");
...
void setup() {
  Serial.begin(115200);
  Serial.println("TOPROAK PRO MAX V7.0 baslatiliyor...");
  ...
  Serial.println("TOPROAK PRO MAX V7.0 - Sistem hazir!");
}
...
void baslangicEkrani() {
  ...
  display.print("Tum Sistemler OK");
  display.display();
...
void serialEvent() {
  while (Serial.available()) {
    ...
    if (c == '\n') {
      ...
      if (command == "status") {
        Serial.println("=== TOPROAK V7.0 STATUS ===");
        Serial.print("Mesafe: ");
        Serial.print(mesafe);
        Serial.println(" cm");
        Serial.print("Gaz: ");
        ...
        Serial.println("TOPROAK V7.0 Komutlari:");
